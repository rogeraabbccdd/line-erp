  ...
  methods: {
    ...
    decode (data) {
      ...
      if (itemIdx >= 0) {
        ...
        if (scannedIdx >= 0 &&
          this.scanned[scannedIdx].history[this.scanned[scannedIdx].history.length - 1].quantity < this.scanned[scannedIdx].scanned
        ) {
          this.scanned[scannedIdx].scanned++
        } else if (scannedIdx < 0 && this.items[itemIdx].history[this.items[itemIdx].history.length - 1].quantity > 0) {
          this.scanned.push({ ...this.items[itemIdx], scanned: 1 })
        } else {
          this.alert.type = 'error'
          this.alert.show = true
          this.alert.text = '超出物品庫存'
        }
      } else {
        this.alert.type = 'error'
        this.alert.show = true
    ...
    async send () {
      ...
      try {
        const items = this.scanned.map((item) => {
          ...
          data.code = item.code
          data.diff = item.scanned * -1
          return data
        })
        await this.$axios.patch('/items/history', { action: 1, items })
        ...
        this.liff.sendMessages([
          {
            type: 'text',
            text: `已於${new Date().toLocaleDateString()}完成出貨`
          }
        ])
        this.liff.closeWindow()
      } catch (error) {
    ...
    plus (item) {
      ...
      if (this.scanned[idx].scanned < this.scanned[idx].history[this.scanned[idx].history.length - 1].quantity) {
